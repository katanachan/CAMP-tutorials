...
'''
def corr_coeff(spktr1, spktr2, time_window,T): #spktr1 & spktr2 is just an array in time where spikes occur & T is recording time of all spike trains
	...
	hist1[(spktr1*1000/dt).astype(int)]=1 #save the array index of type int as 1
	hist2 = np.zeros(len(t)) 
	hist2[(spktr2*1000/dt).astype(int)]=1 #in this we're merely making the histogram
